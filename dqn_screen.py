...
class DQNAgent:
    ...
    def get_action(self,state):
        if np.random.rand()<=self.epsilon:
            return np.random.uniform(low=-1, high=1, size=(self.action_size,))#torch.LongTensor(np.random.uniform(low=-1, high=1, size=(self.action_size,)))
        else:
            with torch.no_grad():
    ...
    def optimize_model(self,done):
        ...
        print(action.resize(self.action_size,BATCH_SIZE)[0].unsqueeze(1).shape)
        #여기서부터는 실행한 행동 a_t에 대한 Q값을 계산하므로 action_batch에서 취한 행동 a_t가 왼쪽이냐 오른쪽이냐에 대한 인덱스를 구하고
        # 이에 대한 Q값을 gather 메서드로 모아온다
        for i in range(action_size):
            current_q = []
            current_q[i] = self.model(state).gather(1, action.resize(self.action_size,BATCH_SIZE)[i].unsqueeze(1)) 
        #current_q = 
